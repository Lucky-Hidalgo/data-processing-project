...
def poblar_personas(facultades, nom_facultad) :
    ...
    for e in facultad :
        ...
        while i < e[3] :
            ...
            dump.append(persona)
            i = i + 1
    data = pd.DataFrame.from_records(dump,
        columns=(
            'nombre_usuario', 
            'clave',
            'nombres',
...
for facultad in facultades.keys() :
    ...
    dump = poblar_personas(facultades, nom_facultad)

    wb = Workbook()

    ws = wb.active

    for r in dataframe_to_rows(dump, header=True):
        ws.append(r)
    nombre = nom_facultad + '.xlsx'
    wb.save(nombre)
